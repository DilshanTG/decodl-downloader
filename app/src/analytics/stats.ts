...
export const calculateDailyStats: DailyStatsJob<never, void> = async (
  ...
  try {
    ...
    // Count users who have made at least one purchase (lifetimeSpentLKR > 0)
    ...
    // Total revenue from all paid payments (in LKR, converted to float)
    const revenueAgg = await context.entities.User.aggregate({
      _sum: {
        lifetimeSpentLKR: true,
      },
    });
    const totalRevenue = (revenueAgg._sum.lifetimeSpentLKR ?? 0) / 100;
